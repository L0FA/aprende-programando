...
import { Menu, X, User, LogOut, ChevronDown, BookOpen, Code, Database, Wrench, Brain, Rocket, Settings } from 'lucide-react';
...
const categories = [
  ...
  { name: 'Backend', slug: 'backend', icon: BookOpen },
  { name: 'Videojuegos', slug: 'videojuegos', icon: Rocket },
  { name: 'Bases de Datos', slug: 'bases-datos', icon: Database },
  { name: 'Herramientas', slug: 'herramientas', icon: Wrench },
  { name: 'IA', slug: 'ia', icon: Brain },
];

function Dropdown({ title, items, icon: Icon }) {
          ...
          <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            <div className="hidden xl:flex items-center justify-between flex-1 px-8">
              <div className="flex items-center gap-1">
                <Link href="/courses" className="px-4 py-2 text-gray-700 dark:text-gray-300 hover:text-primary-500 dark:hover:text-primary-400 transition-colors font-medium rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800">
                  Todos los Cursos
                </Link>
                ...
                <Link href="/guia" className="px-4 py-2 text-gray-700 dark:text-gray-300 hover:text-primary-500 dark:hover:text-primary-400 transition-colors font-medium rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800">
                  Guía de Aprendizaje
                </Link>
                
                {user?.role === 'ADMIN' && (
                  <Link href="/admin" className="px-4 py-2 text-purple-600 dark:text-purple-400 hover:text-purple-500 dark:hover:text-purple-300 transition-colors font-medium rounded-lg hover:bg-purple-50 dark:hover:bg-purple-500/10 flex items-center gap-2">
                  ...
                  <AnimatePresence>
                    {userMenuOpen && (
                      ...
                      >
                        ...
                        <div className="py-2">
                          <Link href="/mi-progreso" onClick={() => setUserMenuOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-gray-700 dark:text-gray-300 hover:bg-primary-50 dark:hover:bg-primary-500/10 hover:text-primary-600 dark:hover:text-primary-400 transition-colors">
                            <Rocket className="w-4 h-4" />
                            Mi Progreso
                          </Link>
                          <Link href="/courses" onClick={() => setUserMenuOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-gray-700 dark:text-gray-300 hover:bg-primary-50 dark:hover:bg-primary-500/10 hover:text-primary-600 dark:hover:text-primary-400 transition-colors">
                            <BookOpen className="w-4 h-4" />
                            Mis Cursos
                          </Link>
                          {user.role === 'ADMIN' && (
                            <Link href="/admin" onClick={() => setUserMenuOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-purple-600 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-500/10 transition-colors">
                              <Settings className="w-4 h-4" />
                              Panel Admin
                            </Link>
                          )}
            ...
            >
              <div className="px-4 py-6 space-y-4">
                <Link 
                  href="/courses" 
                  onClick={() => setIsOpen(false)} 
                  className="flex items-center gap-3 py-3 px-4 text-gray-700 dark:text-gray-300 hover:bg-primary-50 dark:hover:bg-primary-500/10 rounded-xl transition-colors font-medium"
                >
                ...
                {!user && (
                  <div className="pt-4 border-t border-gray-200 dark:border-gray-800 space-y-2">
                    <button 
                      ...
                      className="w-full py-3 px-4 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl transition-colors font-medium text-center"
                    >
                      Iniciar Sesión
                    </button>
                    <button 
                      onClick={() => { openRegister(); setIsOpen(false); }} 
                      className="w-full py-3 px-4 text-sm font-semibold rounded-xl bg-gradient-to-r from-primary-500 to-purple-500 text-white hover:shadow-lg transition-all text-center"
                    >
                      Crear Cuenta
                    </button>
                  </div>
